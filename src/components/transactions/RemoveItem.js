...
import { itemRemover } from "../../redux/actions/dbActions/itemRemover";

function RemoveItem({ id, removeItem, filter, bankName }) {
  const handleClick = () => removeItem(id, filter, bankName);

  return (
    <svg
      className="edit-remove remove"
      viewBox="0 0 512 512"
      onClick={handleClick}
    >
...
const mapDispatchToProps = (dispatch) => {
  return {
    removeItem: (id, filter, bankName) => {
      dispatch(itemRemover(id, filter, bankName));
    },
  };
};
