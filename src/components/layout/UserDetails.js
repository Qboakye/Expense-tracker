...
import { signUserOut } from "../../redux/actions/authActions/signUserOut";

import { auth } from "../../auth/firebase";

function UserDetails({ setIsOpened, signout }) {
  const handleClick = () => signout();

  return (
    <main className="user-pop">
      <div className="user-box">
        <div className="close" onClick={() => setIsOpened(false)}>
          &times;
        </div>
        <svg className="profile-pic" viewBox="0 0 55 55">
          <path
        ...
        <div>
          <button className="sign-out-btn primary" onClick={handleClick}>
            Sign out
          </button>
        </div>
      </div>
